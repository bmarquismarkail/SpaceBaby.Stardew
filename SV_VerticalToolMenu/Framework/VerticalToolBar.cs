    ...
    internal class VerticalToolBar : IClickableMenu
    {
        ...
        public Item RightClick(int x, int y, Item toAddTo, bool playSound = true)
        {
            foreach (ClickableComponent button in this.buttons)
            {
                ...
                {
                    if (verticalItems[toolbarIndex] is Tool && (toAddTo == null || toAddTo is SObject) && (verticalItems[toolbarIndex] as Tool).canThisBeAttached((SObject)toAddTo))
                        return (verticalItems[toolbarIndex] as Tool).attach((SObject)toAddTo);
                    if (toAddTo == null)
                    {
                        if (verticalItems[toolbarIndex].maximumStackSize() != -1)
                        {
                            ...
                            if (verticalItems[toolbarIndex].Stack > 1)
                            {
                                ...
                                {
                                    one.Stack = (int)Math.Ceiling(verticalItems[toolbarIndex].Stack / 2.0);
                                    verticalItems[toolbarIndex].Stack = verticalItems[toolbarIndex].Stack / 2;
                                    goto label_15;
                                }
                            }
                            if (verticalItems[toolbarIndex].Stack == 1)
                                ...
                                --verticalItems[toolbarIndex].Stack;
                            label_15:
                            if (verticalItems[toolbarIndex] != null && verticalItems[toolbarIndex].Stack <= 0)
                                verticalItems[toolbarIndex] = null;
                            if (playSound)
                    ...
                    else if (verticalItems[toolbarIndex].canStackWith(toAddTo) && toAddTo.Stack < toAddTo.maximumStackSize())
                    {
                        if (Game1.isOneOfTheseKeysDown(Game1.oldKBState, new[] { new InputButton(Keys.LeftShift) }))
                        {
                            toAddTo.Stack += (int)Math.Ceiling(verticalItems[toolbarIndex].Stack / 2.0);
                            verticalItems[toolbarIndex].Stack = verticalItems[toolbarIndex].Stack / 2;
                        }
                        else
                        {
            ...
            foreach (ClickableComponent button in this.buttons)
            {
                if (button.containsPoint(x, y))
                {
                    ...
                    if (index >= 0 && index < NUM_BUTTONS && verticalItems[index] != null)
                    {
                        ...
                        this.hoverTitle = verticalItems[index].Name;
                        this.hoverItem = verticalItems[index];
                    }
                }
                else
